...
export default (router: KoaRouter) => {
  ...
  router.post('/movies/:movieId/actors/:personId', async (ctx) => {
    const movie = await Movie.query().findById(ctx.params.movieId)
    const actor = await Person.query().findById(ctx.params.personId)
    const numRelated = await movie.$relatedQuery('actors').relate(actor)
